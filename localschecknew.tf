...
existing_vms = try(
  ...
  final_vm_list = merge(try(local.existing_vms, {} ), { for vm in local.vm_list : vm.name => vm })
...
nic_names = flatten([
  ...
  final_nic_list = merge(try(local.nic_list, {}), { for nic in local.vm_list : nic.name => {} })
...
asg_mapped = merge(try(local.asg_list ,{}), 
{
  ...
  asg_attachment_map = {
    for item  in local.asg_attachment_list : "${item.nic_name}-${item.asg_name}" => item if lookup(local.final_nic_list, item.nic_name, null) != null } 
  }
