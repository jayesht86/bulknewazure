variable "vm_extension" {
  description = <<EOT
    ...
    `vm_extension_name`      - (Required) The name of the virtual machine extension. Changing this forces a new resource to be created.

    `vm_extension_vm_id`     - (Required) The ID of the Virtual Machine. Changing this forces a new resource to be created.

    `vm_extension_publisher` - (Required) The publisher of the extension, available publishers can be found by using the Azure CLI.
    ...
    `vm_extension_minor_version_upgrade` - (Optional) Enables automatic deployment of minor version updates. Defaults to true.

    `vm_extension_auto_upgrade` - (Optional) Should the Extension be automatically updated whenever the Publisher releases a new version? Defaults to false.
  ...
  type = object({
    ...
    # Upgrade
    vm_extension_minor_version_upgrade = optional(bool, true)
    ...
    vm_extension_timeout_create = optional(string, "15m")
    vm_extension_timeout_update = optional(string, "15m")
    vm_extension_timeout_read   = optional(string, "5m")
    vm_extension_timeout_delete = optional(string, "15m")
    ...
    linux_vm_extension_custom_script_enabled                     = optional(bool, false)
